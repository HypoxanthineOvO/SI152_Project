...
if __name__ == "__main__":
    ...
    ## D: f(x) := bᵃ / Γ(a) x^{-a-1} e^{-b/x} where a = 0.5, b = 1
    d = np.random.gamma(0.5, 1, n)
    D = np.diag(d)
    ## H = 40I + LDL^T
    H = 40 * np.identity(n) + L @ D @ L.T
    
    # Generate g: Random vector
    mean_g = np.random.randint(-200, 200)
    std_g = np.random.randint(1, 200)
    ...
    mean_b = np.random.randint(-200, 200)
    std_b = np.random.randint(1, 200)
